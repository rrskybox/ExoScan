...
namespace ExoScan
{
    public static class ExoScanFileManager
    {
        ...
        {
            //Make a list of paths for all target folders
            Configuration cfg = new Configuration();
            //get the exoscan folder name from configuration file
            //make a new one if it didn't exist
            string iExoDir = cfg.CollectionFolderPath;
            List<string> targetDirs = Directory.GetDirectories(iExoDir).ToList();
            return targetDirs;
        }
        ...
        public static List<string> GetTargetNameList()
        {
            ...
            //For ExoScan, this would be the name of the parent directory.
            string iTargetName = Directory.GetParent(iBank).Name;
            List<string> targetDirNames = new List<string>() { iTargetName };
            return targetDirNames;
        }

        public static List<string> GetImgeFilenameList()
        {
            ...
            foreach (string iFile in Directory.GetFiles(iBank, "*.fit").ToList())
        ...
        public static List<DateTime> GetSessionDates(string targetName)
        {
            ...
            //Force consolidation
            var dateList1 = dateList.Distinct();
            return dateList1.ToList();
        }
                ...
                foreach (string f in filterFiles)
                {
                    ...
                    for (int i = 0; i < splitName.Length; i++)
                        if (splitName[i] == "F")
                        {
                            ...
                            targetFilters.Add(Filters.IniLookUpFilterName(filterIndex));
                        }
                }
        ...
        public static (string, string, string, string, string) ParseImageFileName(string imageFilenameWithoutExtension)
        {
            ...
            //  from ExoScan image filenaming conventions
            // 
            char[] sp = { ' ' };
        ...
        public static string GetImagingProfileFilePath()
        {
            //Retrieve path to current imaging profile ini file
            string dirPath = TSX_Resources.GetTSXDirectoryPath();
            IniReadWrite appSettings = new IniReadWrite(dirPath + "AppSettings.ini");
            if (appSettings != null)
                return appSettings.GetData("APP_qsImgSysProfilePath");
            else
                return null;
        }
    }
