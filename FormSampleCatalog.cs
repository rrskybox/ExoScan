...
namespace ExoScan
{
    public partial class FormSampleCatalog : Form
    {
        ...
        public FormSampleCatalog()
        {
            ...
            List<string> targetList = ExoScanFileManager.GetTargetNameList();
            List<string> dateList = ExoScanFileManager.GetAllImageDates();
            ...
            for (int iRow = 0; iRow < imageBankGridView.RowCount; iRow++)
                for (int iCol = 0; iCol < imageBankGridView.ColumnCount; iCol++)
                {
                    ...
                    List<string> imagePaths = ExoScanFileManager.GetTargetSessionPaths(cellTgt, cellDate);
                    ...
                    if (imagePaths.Count > 0)
                    {
                        ...
                        {
                            (string tName, string iDate, string iFilter, string iSeq, string iSet) = ExoScanFileManager.ParseImageFileName(Path.GetFileNameWithoutExtension(path));
                        ...
                        foreach (string filter in filterList)
                            ...
                            "(" +
                            ExoScanFileManager.GetTargetSessionPaths(cellTgt, cellDate, filter).Count().ToString() +
                            ") ";
                    }
                }
        ...
        private void Select_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewCell s in selectedCells)
            {
                if (s.Value != null)
                {
                    ...
                    //Add session sets
                    List<string> imagePaths = ExoScanFileManager.GetTargetSessionPaths(t, Convert.ToDateTime(d));
                    foreach (string f in imagePaths)
                    {
                        //get session set and add to session list
                        (string tName, string iDate, string iFilter, string iSeq, string iSet) = ExoScanFileManager.ParseImageFileName(Path.GetFileNameWithoutExtension(f));
