...
namespace ExoScan
{
    public partial class FormTargetImageAnalysis : Form
    {
        ...
        public void InitializeCollectionData()
        {
            ...
            TargetSelectionBox.Items.Clear();
            foreach (string fd in Directory.EnumerateDirectories(basepath))
            {
                Path.GetFileName(fd);
                TargetSelectionBox.Items.Add(Path.GetFileName(fd));
            }
            TargetSelectionBox.SelectedItem = Path.GetFileName(cfg.CollectionFolderPath);
            ...
            //PrimaryColorBox.SelectedIndex = (int) ColorIndexing.StandardColors.Gp;  
            //DifferentialColorBox.SelectedIndex = (int)ColorIndexing.StandardColors.GBp;
            int pi = Array.IndexOf(colors, colorIndex.PrimaryColorConfiguration);
            int di = Array.IndexOf(colors, colorIndex.DifferentialColorConfiguration);
            PrimaryColorBox.SelectedIndex = pi;
            DifferentialColorBox.SelectedIndex = di;
            ...
            FitsNameBox.Text = "";
            foreach (string f in ExoScanFileManager.GetImgeFilenameList())
            ...
            foreach (string f in ExoScanFileManager.GetTargetNameList())
                TargetPlotBox.Items.Add(f);
            if (TargetPlotBox.Items.Count > 0)
            {
                TargetPlotBox.SelectedIndex = 0;
                foreach (DateTime f in ExoScanFileManager.GetSessionDates(TargetPlotBox.SelectedItem.ToString()))
                {
                    SessionSetList.Items.Add(f.Date.ToString("M/dd/yyyy"));
                }
                if (SessionSetList.Items.Count > 0)
        ...
        {
            ColorIndexing.ColorDataSource priColStd = ColorIndexing.ColorXNameToColorDataSourceEnum(priCol);
            ColorIndexing.ColorDataSource difColStd = ColorIndexing.ColorXNameToColorDataSourceEnum(difCol);
        ...
        public void CalculateTransforms(string catalog)
        {
            ...
            ColorIndexing.ColorDataSource targetStandardColor = ColorIndexing.ColorXNameToColorDataSourceEnum(PrimaryColorBox.Text);
            ColorIndexing.ColorDataSource differentialColor = ColorIndexing.ColorXNameToColorDataSourceEnum(DifferentialColorBox.Text);
                            ...
                            foreach (StarField.FieldLightSource compLS in masterLightSources)
                            {
                                if (compLS.CatalogInfo.Value.GAIACatalogPhotoVar != "Variable")
                                {
                                    ...
                                    if (diffCompIndex != null && priCompIndex != null && diffTgtIndex != null && priTgtIndex != null)
                                    {
                                        ...
                                        double VFld = compLS.ColorStandard(ColorIndexing.ColorXNameToColorDataSourceEnum(CurrentTargetData.PrimaryStandardColor));
        ...
        private void AnalyzeFitsImage(string fitsFilePath)
        {
            ...
            if (TargetCatalogBox.Text.Contains("Gaia"))
                StarADUChart.Titles[0].Text = ((StarField.CatalogData)tgtLightSource.CatalogInfo).GAIACatalogName;
            else
                StarADUChart.Titles[0].Text = ((StarField.CatalogData)tgtLightSource.CatalogInfo).APASSCatalogName;
        ...
        private void TransformTargetImageSet()
        {
            ...
            foreach (FormSampleCatalog.TargetShoot session in SessionList)
            {
                ...
                if (SessionSetList.Items.Count > 0)
                    SessionSetList.SelectedIndex = SessionSetList.Items.IndexOf(CurrentTargetData.SessionDate.ToString("M/dd/yyyy"));

                //PlotPhotometryHistory(CurrentTargetData, CurrentTargetData.SessionDate.ToString());
        ...
        private void TransformButton_Click(object sender, EventArgs e)
        {
            ...
            try { TSX_Resources.FindTarget(TargetSelectionBox.Text); }
            catch
            {
                MessageBox.Show("Target is not found in TheSky catalogs");
                Utility.ButtonGreen(TransformButton);
                IsAnalyzing = false;
                return;
            }
        ...
        private void CloseButton_Click(object sender, EventArgs e)
        {
            ...
            ExoScanFileManager.CloseFitsFile(TSX_Image);
            Close();
            return;
        }
        ...
        private void CollectionSessionDateBox_ValueChanged(object sender, EventArgs e)
        {
            if (!IsInitializing)
            {
                ...
                {
                    Configuration cfg = new Configuration();
                    cfg.TargetListPath = CollectionManagement.OpenCollection(TargetSelectionBox.SelectedItem.ToString());
        ...
        private void PrimaryColorBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (!IsInitializing)
            {
                CurrentTargetData.PrimaryStandardColor = PrimaryColorBox.Text;
                ColorIndexing colorIndex = new ColorIndexing();
                colorIndex.PrimaryColorConfiguration = PrimaryColorBox.SelectedItem.ToString();
            }
            return;
        }
        ...
        private void DifferentialColorBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (!IsInitializing)
            {
                CurrentTargetData.DifferentialStandardColor = DifferentialColorBox.Text;
                ColorIndexing colorIndex = new ColorIndexing();
                colorIndex.DifferentialColorConfiguration = DifferentialColorBox.SelectedItem.ToString();
            }
            return;
        }
        ...
        private void PresetTransformsBox_CheckedChanged(object sender, EventArgs e)
        {
            if (PresetTransformsBox.Checked)
            {
                Configuration cfg = new Configuration();
                if (TargetCatalogBox.Text.Contains("GAIA"))
                {
                    ColorTransformBox.Text = cfg.GaiaColorTransform;
                    MagnitudeTransformBox.Text = cfg.GaiaMagnitudeTransform;
                }
                else
                {
                    ColorTransformBox.Text = cfg.ApassColorTransform;
                    MagnitudeTransformBox.Text = cfg.ApassMagnitudeTransform;
                }
            }
        }
    }
