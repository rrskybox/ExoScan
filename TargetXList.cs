...
namespace ExoScan
{
    ...
    {
        const string TargetListRootX = "ExoScan_List";
        const string TargetListRecordX = "ExoScan_Target";
        const string DifferentialRecordX = "ExoScan_Differential";
        ...
        const string StartTransitX = "StartTransit";
        const string EndTransitX = "EndTransit";
        ...
        public static void AddToTargetXList(string tgtName, double ra, double dec, DateTime lastDate)
        {
            ...
            if (!CollectionManagement.HasCollection(tgtName))
                CollectionManagement.CreateCollection(tgtName);
            if (!File.Exists(cfg.TargetListPath))
            {
                XElement tgtXFile;
                tgtXFile = new XElement(TargetListRootX);
        ...
        public static void AddToTargetXList(string tgtName, double ra, double dec, DateTime lastDate, DateTime startTransit, DateTime endTransit)
        {
            //Adds a Differential type of target into the the target list
            Configuration cfg = new Configuration();
            //If the target list file doesn't exist, then create an empty one
            ...
            var newRef = new XElement(TargetListRecordX,
                   ...
                   new XElement(DecX, dec.ToString()),
                   new XElement(LastDateX, lastDate.ToString()),
                   new XElement(StartTransitX, startTransit.ToString()),
                   new XElement(EndTransitX, endTransit.ToString()));
            XElement acnL = XElement.Load(cfg.TargetListPath);
            acnL.Add(newRef);
            acnL.Save(cfg.TargetListPath);
            return;
        ...
        public static void DeleteFromTargetXList(string tgtName)
        {
            //Removes a target from the target list
            //Load xml list and update data on target
            Configuration cfg = new Configuration();
            //If the target list file exists, read in the targets and delete the one with the target name
            if (File.Exists(cfg.TargetListPath))
            {
                XElement acnL = XElement.Load(cfg.TargetListPath);
                foreach (XElement tn in acnL.Elements(TargetListRecordX).Where(t => ((string)t.Element(NameX) == tgtName)))
                    tn.Remove();
                acnL.Save(cfg.TargetListPath);
            }
            return;
        ...
        public class TargetXDescriptor
        {
            ...
            public TargetXDescriptor(string name, double ra, double dec, DateTime startTransit, DateTime endTransit)
            {
                xTarget = new XElement(TargetListRecordX,
                          ...
                          new XElement(DecX, dec.ToString()),
                          new XElement(LastDateX, DateTime.MinValue),
                          new XElement(StartTransitX, null),
                          new XElement(EndTransitX, null));
            }
            ...
            public DateTime? StartTransit
            {
                ...
                set
                {
                    if (xTarget.Element(StartTransitX) == null)
                        xTarget.Add(new XElement(StartTransitX, value.ToString()));
                    else xTarget.Element(StartTransitX).Value = value.ToString();
                }
            }

            public DateTime? EndTransit
            {
                get
                {
                ...
                set
                {
                    if (xTarget.Element(EndTransitX) == null)
                        xTarget.Add(new XElement(EndTransitX, value.ToString()));
                    else xTarget.Element(EndTransitX).Value = value.ToString();
                }
            }
        }
    }
