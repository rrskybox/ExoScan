...
namespace ExoScan
{
    public class FreshImage
    {
        ...
        public FreshImage(TargetXList.TargetXDescriptor targetX, int filterNumber)
        {
            ...
            string imageDir = agncf.ImageBankFolder;
        ...
        private bool ShootTarget(TargetXList.TargetXDescriptor currentTarget)
        {
            ...
            do
            {
                SetNextImagePath(cfg.ImageBankFolder);
                ...
                LogEntry(currentTarget.Name + ":" + " Banking new image in " + cfg.ImageBankFolder);
                tsx_im.Save();
                repetitions--;
            } while (repetitions > 0);
            LogEntry("Imaging set for filter complete");
            return true;
        }

        //Method to link to ExoScan main form for logging progress.
        private void LogEntry(string upd)
        //Method for projecting log entry to the ExoScan Main Form
        {
            LogUpdate(upd);
            return;
        }
