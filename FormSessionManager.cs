...
namespace ExoScan
{
    public partial class FormSessionManager : Form
    {
        ...
        public FormSessionManager()
        {
            ...
            SessionTab.Enter += new System.EventHandler(UpdateSessionEntries);
            AutoRunTab.Enter += new System.EventHandler(UpdateAutoRunEntries);
        }

        public void InitializeCollection()
        {
            //Initialize ExoScan configuration folders, files, default data, as needed
            Configuration cfg = new Configuration();
            // Get current exoplanet target and load windows
            if (CollectionManagement.HasCollection(CollectionManagement.ActiveCollection()))
                TargetNameBox.Text = CollectionManagement.ActiveCollection();
            else
                TargetNameBox.Text = "No Target Selected";
            ...
            AutoTransitCheckBox.Checked = Convert.ToBoolean(cfg.AutoRunOnExoTransits);
            PreTransitRunUpBox.Value = Convert.ToInt16(cfg.PreTransitRunUp);
            PostTransitRunOutBox.Value = Convert.ToInt16(cfg.PostTransitRunOut);
            DomeHomeAzBox.Value = Convert.ToInt16(cfg.DomeHomeAz);
            ...
            //fTargetForm = new FormCreateTargetList { TopLevel = false };
            //TargetTab.Controls.Add(fTargetForm);
            //fTargetForm.Show();
            InitializeTargetTab();

            //fAutoRunForm = new FormAutoRun { TopLevel = false };
            //AutoRunTab.Controls.Add(fAutoRunForm);
            //fAutoRunForm.Show();
            InitializeAutoRunTab();

            LogEventHandler("");
            LogEventHandler("\r\n" + "********** Initiating ExoScan Session **********");
            LogEventHandler("********** for " + CollectionGroupBox.Text + "**********\r\n");
            return;
        }

        private void LoadTarget(Configuration cfg)
        {
            //Load target
            exoList = new TargetXList();
            if (exoList.TargetXCount > 0)
            {
                List<TargetXList.TargetXDescriptor> exoXlist = exoList.GetTargetXList();
                TargetXList.TargetXDescriptor exoXtgt = exoXlist[0];
                if (exoXtgt.StartTransit != null)
                    StartTransitBox.Text = ((DateTime)exoXtgt.StartTransit).ToString("HH:mm");
                else
                    StartTransitBox.Text = "N/A";
                if (exoXtgt.EndTransit != null)
                    EndTransitBox.Text = ((DateTime)exoXtgt.EndTransit).ToString("HH:mm");
                else
                    EndTransitBox.Text = "N/A";

                if (cfg.AutoRunOnExoTransits == "True")
                {
                    try
                    {
                        cfg.StartUpTime = ((DateTime)exoXtgt.StartTransit - TimeSpan.FromMinutes(Convert.ToInt16(cfg.PreTransitRunUp))).ToString();
                    }
                    catch
                    {
                    ...
                    try
                    {
                        cfg.ShutDownTime = ((DateTime)exoXtgt.EndTransit + TimeSpan.FromMinutes(Convert.ToInt16(cfg.PostTransitRunOut))).ToString();
                    }
                    catch
                    {
                        AutoRunCheckBox.Checked = false;
        ...
        private void StartScan()
        {
            ...
            if (AutoRunCheckBox.Checked)
            {
                ...
                if (AutoRunCheckBox.Checked)
                {
                    if (WatchWeatherCheckBox.Checked) WeatherManagement(false);
                    ...
                    if (WatchWeatherCheckBox.Checked)
                        WeatherManagement(false);
                    if (!abortOn)
                        Launcher.RunStartUp();
                }
                //Check for abort and return if...
                if (abortOn)
                {
            ...
            if (DeviceControl.FocuserStartUp()) LogEventHandler("Initializing Focuser, if needed");
            else LogEventHandler("Focuser initialization failed");
            if (DeviceControl.RotatorStartUp()) LogEventHandler("Initializing Rotator, if needed");
            ...
            if (DeviceControl.HasRotator() && cfg.IsRotateZeroEast == "True")
            {
                LogEventHandler("Checking rotator position for 0 IPA");
                DeviceControl.TelescopePrePosition("East");
                DeviceControl.RotateToImagePA(0);
            }
            ...
            TargetXList.TargetXDescriptor currentTargetXD = (exoList.GetTargetXList())[0];
            ...
            while (currentTargetXD != null)
            {
                ...
                if (Convert.ToBoolean(Launcher.IsAtEnd()))
                {
                ...
                LogEventHandler("-------------------");
                LogEventHandler("Exoplanet target: " + currentTargetXD.Name);
                ...
                foreach (Filters.ActiveFilter af in afList)
                {
                    ...
                    {
                        LogEventHandler(currentTargetXD.Name + " Full image set for filter acquired.");
                    }
                    else
                    {
                ...
                gTriedCount++;
                Show();
                //Check for expiration of sampling period, if not, wait the remaining period time
                DateTime nextSampleTime = currentTargetXD.LastImagingDate + TimeSpan.FromMinutes(Convert.ToDouble(cfg.MinRetakeInterval));
                while (DateTime.Now <= nextSampleTime)
                {
                    LogEventHandler("Awaiting next sample time");
                    //Sleep wait for 1 minute, checking end time and weather every 10 seconds
                    for (int i = 0; i < 6; i++)
                    {
                        //Check for abort and return if...
                        if (abortOn)
        ...
        private void WeatherManagement(bool inSession)
        {
            //Check weather conditions, if enabled
            ...
            if (WatchWeatherCheckBox.Checked)
            {
                ...
                if (!IsWeatherSafe())
                {
                    if (inSession)
                    {
                        ...
                        if (Convert.ToBoolean(cfg.UsesDome))
                        {
                            bool homeStatus = DomeControl.CloseDome();
                            if (homeStatus)
                    ...
                    do
                    {
                        ...
                        if (abortOn)
                        {
                            Utility.ButtonGreen(AbortButton);
                            abortOn = false;
                            break;
                        }
                    } while (!IsWeatherSafe());
                    ...
                    if (IsWeatherSafe())
                    {
                        if (inSession)
                        {
                            ...
                            //Wait for 10 seconds for everything to settle
                            LogEventHandler("Waiting for dome to settle");
                            System.Threading.Thread.Sleep(10000);
                            //Recouple dome
                            if (Convert.ToBoolean(cfg.UsesDome)) DeviceControl.IsDomeCoupled = true;
                        }
        ...
        private void UpdateSessionEntries(object sender, EventArgs e)
        {
            InitializeCollection();
            return;
        }
        ...
        private void StartScanButton_Click(object sender, EventArgs e)
        {
            ...
            if (abortOn)
            {
                StartScanButton.BackColor = Color.LightGreen;
                abortOn = false;
            }
            scanOn = false;
            return;
        ...
        private void AutoTransitCheckBox_CheckedChanged(object sender, EventArgs e)
        {
            //If Autorun on transits is true, then set the autorun values to 1/2 hour before and after transits
            Configuration cfg = new Configuration();
            cfg.AutoRunOnExoTransits = AutoTransitCheckBox.Checked.ToString();
            LoadTarget(cfg);
            return;
        }

        private void PreTransitRunUpBox_ValueChanged(object sender, EventArgs e)
        {
            //Save value in configuration table and update start/stop
            Configuration cfg = new Configuration();
            cfg.PreTransitRunUp = PreTransitRunUpBox.Value.ToString();
            LoadTarget(cfg);
            return;
        }

        private void PostTransitRunOutBox_ValueChanged(object sender, EventArgs e)
        {
            //Save value in configuration table and update start/stop
            Configuration cfg = new Configuration();
            cfg.PostTransitRunOut = PostTransitRunOutBox.Value.ToString();
            LoadTarget(cfg);
            return;
        }

        private void DomeHomeAzBox_ValueChanged(object sender, EventArgs e)
        {
            //Save value in configuration table and update start/stop
            Configuration cfg = new Configuration();
            cfg.DomeHomeAz = DomeHomeAzBox.Value.ToString();
            LoadTarget(cfg);
            return;
        }
